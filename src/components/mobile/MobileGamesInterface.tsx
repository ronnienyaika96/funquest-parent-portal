...
const MobileGamesInterface = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <div className="p-4 space-y-6 pb-24">
        ...
        {/* Featured Games */}
        {featuredGames.length > 0 && <div className="space-y-4">
            ...
            {featuredGames.map(game => <Card key={game.id} className="overflow-hidden border-0 shadow-lg bg-gradient-to-br from-white to-gray-50 transform transition-all duration-300 hover:scale-105 active:scale-95">
                <CardContent className="p-0">
                  <div className={`bg-gradient-to-r ${game.gradient} p-4 text-white relative overflow-hidden`}>
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                      ...
                      <Button onClick={() => playGame(game.id)} className="bg-white text-gray-800 hover:bg-gray-100 rounded-full font-bold px-6 py-2 shadow-lg transform transition-all duration-200 hover:scale-105">
                        <Play className="w-4 h-4 mr-2" />
                        Play Now
                      </Button>
                    </div>
                    
                    {game.completedLevels && <div className="relative z-10 mt-3 pt-3 border-t border-white/20">
                        ...
                        <div className="w-full bg-white/20 rounded-full h-2">
                          <div className="bg-white rounded-full h-2 transition-all duration-500" style={{
                    width: `${game.completedLevels / game.totalLevels * 100}%`
                  }}></div>
                        </div>
                      </div>}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex space-x-3 overflow-x-auto pb-2">
            {categories.map(category => <Button key={category.name} variant={selectedCategory === category.name ? "default" : "outline"} onClick={() => setSelectedCategory(category.name)} className={`flex-shrink-0 rounded-2xl px-4 py-2 min-w-0 transition-all duration-300 ${selectedCategory === category.name ? `bg-purple-500 text-white shadow-lg transform scale-105` : `${category.color} border-0 text-gray-700 hover:scale-105`}`}>
        ...
        <div className="flex items-center space-x-3">
          ...
          <select value={selectedAge} onChange={e => setSelectedAge(e.target.value)} className="flex-1 p-4 rounded-2xl border-2 border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700 font-medium">
        ...
        {/* Games Grid */}
        {regularGames.length > 0 && <div className="space-y-4">
            <h3 className="text-xl font-bold text-gray-800">More Adventures</h3>
            <div className="grid grid-cols-1 gap-4">
              {filteredGames.filter(game => !game.featured).map(game => <Card key={game.id} className="overflow-hidden border-0 shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-102 active:scale-98">
                  <CardContent className="p-0">
                    <div className="flex">
                      ...
                      <div className="flex-1 p-4">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Button onClick={() => playGame(game.id)} size="sm" className="bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full font-bold px-4 py-2 shadow-md hover:shadow-lg transform transition-all duration-200 hover:scale-105">
        ...
        {/* No Results */}
        {filteredGames.length === 0 && <div className="text-center py-16">
